...
@section('content')
	<h1>Editar {{ $preferencia->nombre }}</h1>

	<!-- if there are creation errors, they will show here -->
    {{ HTML::ul($errors->all()) }}

    {{ Form::model($preferencia, array('route' => array('preferencias.update', $preferencia->id), 'method' => 'PUT', 'files'=>true)) }}
...
<!--
        ...
        <div class="form-group">
            {{ Form::label('premiostrecant', 'Cantidad de Premios Tres') }}
            {{ Form::text('premiostrecant', Input::old('premiotrescant'), array('class' => 'form-control')) }}
        </div> 
        ...
        {{ Form::submit('Editar la preferencia!', array('class' => 'btn btn-primary')) }}
